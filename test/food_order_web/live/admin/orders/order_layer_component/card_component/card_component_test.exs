defmodule FoodOrderWeb.Admin.CardComponentTest do
  ...
  describe "test card component" do
    ...
    test "load card", %{conn: conn} do
      ...
      assert has_element?(
               view,
               "[data-role=item][data-id=NOT_STARTED-#{order.id}-#{hd(order.items).id}]"
             )

      assert has_element?(
               view,
               "[data-role=item-details][data-id=NOT_STARTED-#{order.id}-#{hd(order.items).id}]",
               "#{hd(order.items).quantity} - #{hd(order.items).product.name}"
             )

      assert has_element?(
               view,
               "[data-role=item-price][data-id=NOT_STARTED-#{order.id}-#{hd(order.items).id}]",
               "#{Money.to_string(hd(order.items).product.price)}"
             )
      ...
      assert has_element?(
               view,
               "[data-role=total-cost-amount][data-id=NOT_STARTED-#{order.id}]",
      ...
      assert has_element?(
               view,
               "[data-role=total-items-amount][data-id=NOT_STARTED-#{order.id}]",
               "#{order.total_quantity}"
             )

      assert has_element?(
               view,
               "[data-role=customer][data-id=NOT_STARTED-#{order.id}]",
               "#{order.user.email}"
             )
    end
  end
